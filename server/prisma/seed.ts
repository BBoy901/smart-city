...
async function main() {
  ...
  const shop = await prisma.shop.upsert({
    ...
    create: {
      ...
      name: "ABC Shoes",
      description:
        "Your trusted shoe store in Kariakoo. Sneakers, formal shoes, and more.",
      phone: "+255712345678",
      ...
      shopCategories: fashionCat
        ? { create: [{ categoryId: fashionCat.id }] }
        : undefined,
    },
    update: {},
  });
  ...
  const sampleProducts = [
    {
      name: "Nike Air Max",
      description: "Classic Nike Air Max sneakers, comfortable and stylish.",
      price: 85000,
    ...
    },
    {
      ...
      description: "Elegant black leather formal shoes for office.",
      price: 65000,
      categoryId: fashionCat?.id,
    },
    {
      name: "Canvas Sneakers",
      description: "Casual canvas sneakers, multiple colors available.",
      price: 35000,
      categoryId: fashionCat?.id,
    },
  ];
  ...
  const techProducts = [
    {
      ...
      description: "Latest Samsung mid-range smartphone.",
      price: 650000,
      categoryId: electronicsCat?.id,
    },
    {
      name: "iPhone 13",
      description: "Apple iPhone 13, excellent condition.",
      price: 1200000,
      categoryId: electronicsCat?.id,
    },
    {
  ...
  for (const p of techProducts) {
    const existing = await prisma.product.findFirst({
      where: { name: p.name, shopId: techShop.id },
    });
    if (!existing) {
      await prisma.product.create({
        data: {
