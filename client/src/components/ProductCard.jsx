  ...
  return (
    <div className="product-card">
      ...
      <div className="product-card-actions">
        <button
          className="btn-icon"
          onClick={() => requireLogin("like") && onLike?.(product.id)}
          title="Like"
        >
          <Heart
            size={18}
        ...
        <button
          className="btn-icon"
          onClick={() => requireLogin("save") && onSave?.(product.id)}
          title="Save"
        >
          <Bookmark
            size={18}
        ...
        <Link
          to={
            isAuthenticated
              ? `/messages?to=${product.shop?.sellerProfile?.user?.id}&product=${product.id}`
              : "/login"
          }
          state={
            !isAuthenticated
              ? { from: `/product/${product.id}`, action: "message" }
              : undefined
          }
          className="btn-icon"
          title="Message"
