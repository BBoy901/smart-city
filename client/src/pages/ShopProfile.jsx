    ...
    <div className="page-no-nav">
      ...
      <div className="seller-info">
        ...
        {shop.shopCategories?.length > 0 && (
          <div className="chips-row" style={{ marginTop: 8 }}>
            {shop.shopCategories.map((sc) => (
              <span key={sc.categoryId} className="chip">
                {sc.category.name}
              </span>
            ))}
          </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          {shop.phone && (
            <a href={`tel:${shop.phone}`} className="btn btn-primary btn-sm">
              <Phone size={16} /> Call
            </a>
          )}
          {sellerUser?.id && (
            <Link
              to={`/messages?to=${sellerUser.id}`}
        ...
        <div className="section">
          ...
          {shop.phone && (
            <div style={{ marginTop: 16 }}>
              ...
              <a href={`tel:${shop.phone}`} style={{ color: "var(--primary)" }}>
                {shop.phone}
              </a>
            </div>
          )}
