...
export default function SellerHome() {
  ...
  const loadDashboard = () => {
    ...
    Promise.all([api.getMyShops(), api.getMyProducts().catch(() => [])])
      .then(([s, p]) => {
        setShops(s);
        setProducts(p);
      })
      .catch((err) => {
        setError(err.message || "Could not load your shop. Please try again.");
      })
...
/>
        ...
        {!error && shops.length === 0 ? (
          ...
          >
            ...
            <Link to="/seller/setup" className="btn btn-primary">
              Create Shop
            </Link>
          </div>
        ) : (
